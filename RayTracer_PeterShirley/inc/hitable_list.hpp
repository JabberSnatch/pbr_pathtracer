...
struct hitable_list : public hitable
{
	...
	hitable_list(const std::vector<std::shared_ptr<hitable>>& _list)
		:list{_list}
	{}
...
hitable_list::hit(const ray& _r, float _tMin, float _tMax, hit_record& _rec) const
{
	...
	for (auto item : list)
	{
		if (item->hit(_r, _tMin, closest_so_far, temp_record))
		{
			hit_anything = true;
...
hitable_list::bounding_box(float _t0, float _t1, aabb& _box) const
{
	...
	aabb temp_box;
	bool first_true = list[0]->bounding_box(_t0, _t1, temp_box);
	if (!first_true)
		return false;
	else
		_box = temp_box;
	...
	{
		if (list[i]->bounding_box(_t0, _t1, temp_box))
